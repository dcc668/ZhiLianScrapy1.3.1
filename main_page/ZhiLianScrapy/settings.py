...
DUPEFILTER_CLASS = "scrapy_redis.dupefilter.RFPDupeFilter"#使用scrapy-redis里面的去重组件.
SCHEDULER = "scrapy_redis.scheduler.Scheduler"# 使用scrapy-redis里面的调度器
SCHEDULER_PERSIST = True
# 指定排序爬取地址时使用的队列，
SCHEDULER_QUEUE_CLASS = "scrapy_redis.queue.SpiderPriorityQueue"# 默认的 按优先级排序(Scrapy默认)，由sorted set实现的一种非FIFO、LIFO方式。
...
SCHEDULER_PERSIST = True# 允许暂停后,能保存进度
